public class VoyageStandard extends VoyageBase {

    public VoyageStandard(String type, String id, String from, String to, String rows, String price, String refundCut,
            String outputFile)
            throws Exception {
        this.type = type;
        this.from = from;
        ...
        this.leftSeatCount = this.rows * 2;
        this.rightSeatCount = this.rows * 2;
        FileIO.writeFile(outputFile, String.format(
                "Voyage %d was initialized as a standard (2+2) voyage from %s to %s with %.2f TL priced %d regular seats. Note that refunds will be %d%% less than the paid amount.",
                this.id, this.from, this.to, this.price, this.leftSeatCount + this.rightSeatCount, this.refundCut),
                true, true);
    }
